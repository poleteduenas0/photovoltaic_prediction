...
Este script implementa un modelo LSTM bidireccional para la predicción de horas
de sol utilizando datos meteorológicos de AEMET. Incluye evaluación de métricas
y guardado de resultados en base de datos.
"""
import os
...
def data_obtention():
    """
    Obtain AEMET data from the PostgreSQL database.
    """
    load_dotenv()
...
def data_preprocessing(df, target_var='sol'):
    """
    Preprocess the AEMET data for the LSTM model.
    
    Args:
        df: DataFrame with AEMET data
        target_var: Target variable to predict (default: 'sol')
    ...
    cols_to_drop = ['estacion', 'mes', 'dia', 'dia_semana', 'es_fin_semana']
...
def create_sequences(data, target_col_idx, seq_length):
    """
    Create sequences for LSTM from data.
...
def build_model(input_shape):
    """
    Build LSTM model for solar radiation prediction.
    """
    model = Sequential([
...
def evaluate_model_and_save_metrics(y_true, y_pred, model_name, engine):
    ...
    try:
        ...
        csv_filename = f'models/metrics_results/aemet_lstm_metrics_{timestamp_str}.csv'
...
def plot_results(test_dates, y_test, y_pred, future_dates, future_preds, target_var):
    ...
    plt.title(f'{target_var.capitalize()} Prediction and Forecast')
    plt.xlabel('Date')
    plt.ylabel(f'{target_var.capitalize()} (hours)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    
    # Save the figure
    plt.savefig(f'models/visualizations/aemet/lstm_{target_var}_forecast.png')
    # plt.show()
...
def main(n_days=7):
    """
    Main function to run the LSTM model for solar radiation prediction.
    """
    n_days=7
    # Set target variable
    target_var = 'sol'
    ...
    # 3) Create sequences
    seq_length = n_days  # Use 7 days as input window
    ...
    evaluate_model_and_save_metrics(y_test_inv, y_pred_inv, 'LSTM', engine)
    ...
    os.makedirs('models/saved_models/aemet', exist_ok=True)
    model.save(f'models/saved_models/aemet/lstm_{target_var}_model.h5')
    
    with open(f'models/saved_models/aemet//scaler_all_{target_var}.pkl', 'wb') as f:
        pickle.dump(scaler_all, f)
    
    with open(f'models/saved_models/aemet/scaler_{target_var}.pkl', 'wb') as f:
        pickle.dump(scaler_target, f)
    
    print(f"\nModel and scalers saved to 'models/saved_models/aemet/' directory")
    ...
    plt.tight_layout()
    plt.savefig(f'models/visualizations/aemet/lstm_{target_var}_training_history.png')
    # plt.show()
    ...
    future_df.to_sql(f'aemet_forecast_{target_var}', engine, if_exists='replace', index=True)
    print(f"Predicted data saved to 'aemet_forecast_{target_var}' table in PostgreSQL.")
