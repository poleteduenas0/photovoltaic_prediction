...
def evaluate_model_and_save_metrics(y_true, y_pred, model_name, engine):
    ...
    try:
        ...
        csv_filename = f'models/metrics_results/aemet_bilstm_metrics_{timestamp_str}.csv'
...
def plot_results(test_dates, y_test, y_pred, future_dates, future_preds, target_var):
    ...
    # Save the figure
    plt.savefig(f'models/visualizations/aemet/bilstm_{target_var}_forecast.png')
    plt.close()
...
def main(n_days=7):
    ...
    model.save(f'models/saved_models/aemet/bilstm_{target_var}_model.h5')
    
    with open(f'models/saved_models/aemet/scaler_all_{target_var}_bilstm.pkl', 'wb') as f:
        pickle.dump(scaler_all, f)
    
    with open(f'models/saved_models/aemet/scaler_{target_var}_bilstm.pkl', 'wb') as f:
        pickle.dump(scaler_target, f)
    
    print(f"\nBiLSTM model and scalers saved to 'models/saved_models/aemet/' directory")
    ...
    plt.tight_layout()
    plt.savefig(f'models/visualizations/aemet/bilstm_{target_var}_training_history.png')
    plt.close()
    ...
    future_df.to_sql(f'aemet_forecast_{target_var}_bilstm', engine, if_exists='replace', index=True)
    print(f"Predicted data saved to 'aemet_forecast_{target_var}_bilstm' table in PostgreSQL.")
