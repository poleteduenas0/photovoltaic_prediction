...
Este script implementa modelos ARIMA y ARMAX para la predicción de producción fotovoltaica.
Utiliza datos históricos de producción y variables meteorológicas como exógenas.
Compara el rendimiento de ambos modelos y guarda las métricas de evaluación.
"""
...
def data_obtention():
    """Obtener datos de producción desde PostgreSQL"""
    load_dotenv()
    ...
    prod_table = os.getenv('PRODUCTION_POSTGRES_TABLE_TREATED')
    df_prod = pd.read_sql(f"SELECT * FROM {prod_table} ORDER BY fecha", engine)
    
    # Obtener datos meteorológicos como variables exógenas
    aemet_table = os.getenv('AEMET_POSTGRES_TABLE_TREATED')
    df_aemet = pd.read_sql(f"SELECT fecha, sol, tmed, tmax, tmin, prec FROM {aemet_table} ORDER BY fecha", engine)
    
    # Combinar datos
    df_prod['fecha'] = pd.to_datetime(df_prod['fecha'])
    df_aemet['fecha'] = pd.to_datetime(df_aemet['fecha'])
    
    df_combined = pd.merge(df_prod, df_aemet, on='fecha', how='inner')
    df_combined.set_index('fecha', inplace=True)
    
    print(f"Datos combinados. Forma: {df_combined.shape}")
    return df_combined, engine
...
def prepare_data(df, target_var='Totalproduction'):
    ...
    # Variables exógenas
    exog_vars = ['hrmin', 'hrmedia', 'sol', 'tmed', 'tmax', 'tmin', 'prec']
...
def save_metrics_to_db(metrics_list, engine):
    ...
    try:
        ...
        csv_filename = f'models/metrics_results/production_arima_metrics_{timestamp_str}.csv'
...
def plot_results(y_true, y_pred_arima, y_pred_armax, future_dates, 
                forecast_arima, forecast_armax, target_var='Totalproduction'):
    """Crear visualizaciones de los resultados"""
    
    os.makedirs('models/visualizations/production', exist_ok=True)
    ...
    plt.tight_layout()
    plt.savefig('models/visualizations/production/arima_armax_comparison.png', dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('models/visualizations/production/metrics_comparison.png', dpi=300)
    plt.close()
    
    print("Visualizaciones guardadas en 'models/visualizations/production/'")

def save_predictions_and_plot(future_dates, future_preds, target_var='Totalproduction'):
    """
    ...
    try:
        ...
        # Nombre del archivo con timestamp
        timestamp_str = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
        csv_filename = f'models/metrics_results/production_arima_predictions_{timestamp_str}.csv'
    ...
    try:
        ...
        plt.figure(figsize=(12, 6))
        plt.plot(future_dates, future_preds, marker='o', linestyle='-', linewidth=2, markersize=6, color='green')
        plt.title(f'Predicción ARIMA - {target_var} (7 días)', fontsize=14)
        plt.xlabel('Fecha', fontsize=12)
        plt.ylabel('Producción Total', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.xticks(rotation=45)
        plt.tight_layout()
        
        # Guardar PNG
        png_filename = f'models/visualizations/production/arima_{target_var}_forecast_7d.png'
...
def main(n_days=7):
    ...
    if arima_model:
        try:
            predictions_arima = arima_model.forecast(steps=len(test_data))
        except Exception as e:
    ...
    if armax_model and exog_vars:
        try:
            predictions_armax = armax_model.forecast(
                steps=len(test_data), 
    ...
    # 11. Guardar predicciones futuras
    if len(future_arima) > 0:
        ...
        future_df_arima.to_sql(
            f'production_forecast_{target_var}_arima', 
            engine, 
            if_exists='replace', 
            index=False
        )
    ...
    if len(future_armax) > 0:
        ...
        future_df_armax.to_sql(
            f'production_forecast_{target_var}_armax', 
            engine, 
            if_exists='replace', 
            index=False
        )
