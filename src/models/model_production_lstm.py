...
def evaluate_model_and_save_metrics(y_true, y_pred, model_name, engine):
    ...
    try:
        ...
        csv_filename = f'models/metrics_results/production_lstm_metrics_{timestamp_str}.csv'
...
def main(n_days=5):
    ...
    evaluate_model_and_save_metrics(y_test_inv, preds_test_inv, 'LSTM', engine)
    ...
    plt.title('Producción real vs predicha y forecast 7 días')
    plt.xlabel('Fecha')
    plt.ylabel('Total Production')
    plt.legend()
    plt.tight_layout()
    plt.savefig('models/visualizations/production/lstm_forecast_7d.png')
    # plt.show()

    # 8) Guardar modelo y scalers
    model.save('models/saved_models/production/lstm_model_prod.h5')
    with open('models/saved_models/production/scaler_all.pkl','wb') as f: pickle.dump(scaler_all, f)
    with open('models/saved_models/production/scaler_prod.pkl','wb') as f: pickle.dump(scaler_prod, f)
    ...
    future_df.to_sql('production_forecast_totalproduction', engine, if_exists='replace', index=True)
