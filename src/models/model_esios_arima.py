...
Este script implementa modelos ARIMA y ARMAX para la predicción de precios de excedente
eléctrico utilizando datos de ESIOS. Incorpora variables del mercado eléctrico como
exógenas y compara el rendimiento de ambos modelos.
"""
...
def data_obtention():
    """Obtener datos de ESIOS desde PostgreSQL"""
    load_dotenv()
...
def prepare_data(df, target_var='precio_excedente'):
    ...
    exog_vars = ['Generacion_no_renovable', 'Carbon', 'Eolica', 
                 'Generacion_renovable', 'Solar_fotovoltaica', 'Demanda', 'Ciclo_combinado']
...
def save_metrics_to_db(metrics_list, engine):
    ...
    try:
        ...
        csv_filename = f'models/metrics_results/esios_arima_metrics_{timestamp_str}.csv'
...
def plot_results(y_true, y_pred_arima, y_pred_armax, future_dates, 
                forecast_arima, forecast_armax, target_var='precio_excedente'):
    """Crear visualizaciones de los resultados"""
    
    os.makedirs('models/visualizations/esios', exist_ok=True)
    ...
    plt.tight_layout()
    plt.savefig('models/visualizations/esios/arima_armax_comparison.png', dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('models/visualizations/esios/metrics_comparison.png', dpi=300)
    plt.close()
    
    print("Visualizaciones guardadas en 'models/visualizations/esios/'")

def save_predictions_and_plot(future_dates, future_preds, target_var='precio_excedente'):
    """
    ...
    try:
        ...
        # Nombre del archivo con timestamp
        timestamp_str = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
        csv_filename = f'models/metrics_results/esios_arima_predictions_{timestamp_str}.csv'
    ...
    try:
        ...
        plt.figure(figsize=(12, 6))
        plt.plot(future_dates, future_preds, marker='o', linestyle='-', linewidth=2, markersize=6, color='orange')
        plt.title(f'Predicción ARIMA - {target_var.replace("_", " ").title()} (7 días)', fontsize=14)
        plt.xlabel('Fecha', fontsize=12)
        plt.ylabel(f'{target_var.replace("_", " ").title()}', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.xticks(rotation=45)
        plt.tight_layout()
        
        # Guardar PNG
        png_filename = f'models/visualizations/esios/arima_{target_var}_forecast_7d.png'
...
def main(n_days=7):
    ...
    if len(future_arima) > 0:
        ...
        future_df_arima.to_sql(
            f'esios_forecast_{target_var}_arima', 
            engine, 
            if_exists='replace', 
            index=False
        )
    ...
    if len(future_armax) > 0:
        ...
        future_df_armax.to_sql(
            f'esios_forecast_{target_var}_armax', 
            engine, 
            if_exists='replace', 
            index=False
        )
