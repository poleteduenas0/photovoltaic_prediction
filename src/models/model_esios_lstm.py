...
Este script implementa un modelo LSTM bidireccional para la predicción de precios
...
def data_preprocessing(df, target_var='precio_excedente'):
    """
    Preprocess the ESIOS data for the LSTM model.
...
def evaluate_model_and_save_metrics(y_true, y_pred, model_name, engine):
    ...
    try:
        ...
        csv_filename = f'models/metrics_results/esios_lstm_metrics_{timestamp_str}.csv'
...
def plot_results(test_dates, y_test, y_pred, future_dates, future_preds, target_var):
    ...
    # Save the figure
    plt.savefig(f'models/visualizations/esios/lstm_{target_var}_forecast.png')
    # plt.show()
...
def main(n_days=5):
    ...
    # 3) Create sequences
    seq_length = n_days  # Use 14 days as input window
    ...
    model.save(f'models/saved_models/lstm_{target_var}_model.h5')
    
    with open(f'models/saved_models/esios/scaler_all_{target_var}.pkl', 'wb') as f:
        pickle.dump(scaler_all, f)
    
    with open(f'models/saved_models/esios/scaler_{target_var}.pkl', 'wb') as f:
        pickle.dump(scaler_target, f)
    
    print(f"\nModel and scalers saved to 'saved_models/esios/' directory")
    ...
    plt.tight_layout()
    plt.savefig(f'models/visualizations/esios/lstm_{target_var}_training_history.png')
    # plt.show()
